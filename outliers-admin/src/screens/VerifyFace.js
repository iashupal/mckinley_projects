...
import { GET_USER_URL, PATCH_FACE_VERIFY_DOCUMENT_URL } from "../utils/endpoints";
...
class VerifyFace extends Component {
  constructor(props) {
    super(props);
    ...
    this.columns = [
      ...
      {
        title: "Appearance Image",
        dataIndex: "appearanceImageUrl",
        width: "15%",
        render: (text, record) => (
          <div>
            <img
              style={{ width: "120px", height: "120px", cursor: "pointer", objectFit: "cover" }}
              src={record.appearanceImageUrl ? record.appearanceImageUrl : dummy}
              alt="Document_Image"
              onClick={() => this.showFaceModal(record)}
            />
          </div>
        )
      },
      {
        title: "Status",
        dataIndex: "appearanceVerified",
        width: "8%",
        key: "appearanceVerified",
        render: (text, record) => <span>{record.appearanceVerified ? record.appearanceVerified : ""}</span>
      },
      {
        title: "Actions",
        key: "actions",
        ...
        render: (text, record) => (
          <span>
            {record.appearanceVerified === "false" ? (
              <Fragment>
                <Popconfirm
                  title="Are you sure, you want to verify face appearance?"
                  onConfirm={() => this.VerifyAppearance(record._id, "accepted")}
                  okText="Yes"
                  cancelText="No"
                >
            ...
            {record.appearanceVerified === "true" ? (
              <Fragment>
                <Divider type="vertical" />
                <Popconfirm
                  title="Are you sure, you want to reject face appearance?"
                  onConfirm={() => this.VerifyAppearance(record._id, "rejected")}
                  okText="Yes"
                  cancelText="No"
                >
    ...
    axios
      ...
      .then(res => {
        ...
        res.data.allUsers.forEach(data => {
          data.appearanceVerified = data.appearanceVerified.toString();
        });
  ...
  showFaceModal = record => {
    Modal.success({
      title: "Face apperarance",
      content: (
        <div
          style={{
            width: "100%",
  ...
  VerifyAppearance(_id, status) {
    axios
      .patch(
        `${PATCH_FACE_VERIFY_DOCUMENT_URL}`,
        { userId: _id, status },
        {
          headers: {
            Authorization: token,
    ...
    return (
      <div>
        <Card
          ...
          size="small"
          title={searched ? `Results for "${searchText}"` : "Verify Face Document"}
          extra={[
            <Popover
              content={
                <div>
