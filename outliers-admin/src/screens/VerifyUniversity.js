...
class VerifyUniversity extends Component {
  constructor(props) {
    super(props);
    ...
    this.columns = [
      ...
      {
        title: "Status",
        dataIndex: "universityVerified",
        key: "universityVerified",
        width: "15%"
      },
      {
        title: "University Doc.",
        dataIndex: "universityDocumentOrEmail",
        width: "15%",
        render: (text, record) => (
          <div>
            {record.universityVerifyMode.toLowerCase() === "document" ? (
              ...
              `verified ${record.universityVerifyMode}`
            )}
          </div>
        )
      },
      {
        ...
        render: (text, record) => (
          <span>
            {record.universityVerified !== "accepted" && record.universityVerifyMode !== "email" && (
              <Fragment>
                <Popconfirm
                  title="Are you sure, you want to approve this university document?"
                  onConfirm={() => this.VerifyUniversityDocument(record._id, "accepted")}
                  okText="Yes"
                  cancelText="No"
                >
            ...
            {record.universityVerified !== "rejected" && record.universityVerifyMode !== "email" && (
              <Fragment>
                <Popconfirm
                  title="Are you sure, you want to reject this university document?"
                  onConfirm={() => this.VerifyUniversityDocument(record._id, "rejected")}
                  okText="Yes"
                  cancelText="No"
                >
    ...
    axios
      ...
      .then(res => {
        ...
        res.data.allUsers.forEach(data => {
          data.universityVerified = data.universityVerified.toString();
        });
  ...
  VerifyUniversityDocument(_id, status) {
    axios
      .patch(
        `${PATCH_VERIFY_DOCUMENT_URL}`,
        { userId: _id, type: "university", status },
        {
          headers: {
            Authorization: token,
    ...
    return (
      <div>
        <Card
          ...
          size="small"
          title={searched ? `Results for "${searchText}"` : "Verify University Document"}
          extra={[
            <Popover
              content={
                <div>
