...
class MemoForm extends Component {
  render() {
    const { classes, handleCommonAlertConfirmSet, setReduxValues, checkInputData, formMode, memo } = this.props;
    ...
    const cardTitle = () => {
      if (isDetail) return '송무 메모 상세';
      if (isCreate) return '송무 메모 등록';
      return '송무 메모 수정';
    };
