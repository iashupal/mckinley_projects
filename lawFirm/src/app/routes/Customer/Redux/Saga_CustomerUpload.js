...
import { CUSTOMER_UPLOAD_SAVE_DRAFT } from './ActionType';
...
function* saveDraft(action) {
  ...
  const { companyList } = customerMng.upLoad;
  const { totalData, handleTableData, type } = action.payload;

  const data = yield call(PostCall, '/lawFirm/customer/cusotmerUpload', {
...
export default function* rootSaga() {
  yield all([takeEvery(CUSTOMER_UPLOAD_SAVE_DRAFT, saveDraft)]);
}
